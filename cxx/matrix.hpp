...
public:
    static Matrix<Type> StrassenMultiply(const Matrix<Type>& A, const Matrix<Type>& B)
    {
        if (A.size() == B.size() && A.size() == 1) {
            ...
            auto A22 = A.part(A.row() / 2 + 1, A.column() / 2 + 1, A.row(), A.column());
            ...
            auto B22 = B.part(B.row() / 2 + 1, B.column() / 2 + 1, B.row(), B.column());
    ...
    static Matrix<Type> NaiveMultiply(const Matrix<Type>& A, const Matrix<Type>& B)
    {
...
public:
    ...
    Matrix(std::initializer_list<std::initializer_list<Type>> const& IM)
    {
        ...
        for (auto il : IM) {
            ...
            c = std::max(static_cast<typename std::vector<Type>::size_type>(c), tmp.size());
            M.push_back(tmp);
        }
        alloc(r, c);
        fill(1, 1, r, c, 0);
    ...
    inline Matrix<Type> operator*(const Matrix<Type>& rhs) const
    {
        if (this->_column != rhs.row()) {
            ...
            int align = std::ceil(std::log2(std::max({ this->row(), this->column(), rhs.row(), rhs.column() })));
            if (std::pow(align, 2.73) * 1.1 >= this->row() * this->column() * rhs.column()) {
                ...
                return StrassenMultiply(alignA, alignB).part(1, 1, this->row(), rhs.column());
            }
        }
    }
    inline Matrix<Type> quickPow(const Matrix<Type>& Base, size_t x) const
    {
        if (x > 0) {
            auto res = std::move(Base);
            auto a = *this;
            while (x) {
                if (x & 1) {
                    res = res * a;
